...
async function doHardWork()
{
	...
	{
		theCon = new ethers.Contract(f_1_add, farabi, signer);
		th = await theCon.doHardWork()//.send({from:window.ethereum.selectedAddress},(e, r) => {console.log(r)}).then((c)=>{console.log(c);gs();});
...
async function DrefreshFarm()
{
	try
	{
		...
		await Promise.all([ts,vl,io]).then(d=>{
			...
			typeof $("est-hvst2") == null ? true: $("est-hvst2").innerHTML = "<b>"+fornum(Number(oi[i-2])*0.01,decEARN)+"</b>"
		})
	}
